...
def main():
    # Resolve the current directory and project root directory
    curr_dir = pathlib.Path(__file__).resolve()
    project_root = curr_dir.parents[2]

    # Use sys.argv[1] to get the relative path to the processed data directory from dvc.yaml
    # Ensure the relative path is joined correctly to the project root
    data_path = project_root / pathlib.Path(sys.argv[1])

    # Define the output path for the models
    output_path = project_root / "models"
    output_path.mkdir(parents=True, exist_ok=True)

    TARGET = "trip_duration"

    # Make sure to use the pathlib object to open the file
    ...
    # Use the pathlib object to save the model
